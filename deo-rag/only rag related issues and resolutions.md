...
# 6. Robust Legal Case Summarization with Context Expansion and Deduplication Fix

**Problem**: The RAG pipeline correctly identified source documents but returned "Insufficient evidence" or very thin answers when asked for long-form summaries (e.g., 500-word summaries of a 50-page case).

**Root Causes & Resolutions**:
1. **Shallow Fusion Keys (Data Loss)**: In `hybrid_retrieval.py`, the `_doc_key` used to fuse BM25 and vector results relied on only the first 80 characters of content. Since legal documents often share identical headers (e.g., "IN THE SUPREME COURT OF INDIA..."), unique chunks were being merged and discarded as duplicates.
   - **Fix**: Switched to full content MD5 hashing for `_doc_key` to ensure every unique chunk is preserved during fusion.
...
**Result**: High-quality, stable summaries for all legal cases in the library, with typical answer lengths increasing from <1000 to **2000-3000 characters**.
