...
## Phase 2: Retrieval and Chunking Enhancements (Completed)

Goal: Improve retrieval precision and recall using advanced chunking and multi-stage retrieval.

### 9. Structure-aware Chunking (Docling)
Replace recursive character splitting with layout-aware chunking.
- Use Docling's `HybridChunker` to respect section boundaries and keep tables atomic.
- Preserve hierarchical metadata (headings, page numbers) for better context.

### 10. Multi-vector Indexing
Store multiple representations for each chunk to capture different semantic aspects.
- **Full Text**: The original chunk content.
- **Summary**: A concise one-sentence summary generated by local LLM.
- **Keywords**: Top terms extracted from the chunk.
- All representations share a `parent_chunk_id` for context reconstruction.

### 11. Advanced Query Pipeline
Add a pre-retrieval processing layer.
- **Query Expansion**: Expand queries with domain synonyms and Hindi-English variants.
- **Query Classification**: Detect keyword-heavy vs semantic queries for dynamic weighting.

### 12. Dynamic Hybrid Retrieval
Improve BM25 and Dense fusion.
- Adjust weights between BM25 and Vector search based on query classification.
- Keywords/IDs get higher BM25 weight; conceptual questions get higher vector weight.

### 13. Cross-encoder Reranking
Add a local reranker stage for top candidates.
- Use `cross-encoder/ms-marco-MiniLM-L-6-v2` to rescore top 30-50 hits.
- Take top 5-10 for the final generation context.

### 14. Parent Context Expansion
Reconstruct full context from matched representations.
- If a summary or keyword chunk is retrieved, automatically fetch the 'full' text representation for the LLM context.
